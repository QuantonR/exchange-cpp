...
Order::Order(const OrderData& orderData, Limit* parentLimit, uint64_t newOrderId)
    : orderData(orderData), parentLimit(parentLimit), nextOrder(nullptr), prevOrder(nullptr), orderId(newOrderId) {
...
void Order::setExecutedQuantity(int executedQuantity){
    orderData.executedQuantity = executedQuantity;
}

void Order::setAvgPrice(int avgPrice){
...
int Order::getExecutedQuantity() const{
    return orderData.executedQuantity;
}

int Order::getAvgPrice() const{
    return orderData.avgPrice;
}
